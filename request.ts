...
export async function fetchWithUA(
  ...
  req.headers.set(
    "user-agent",
    `trim21/bangumi-episode-ics/cf-workers/${pkg.version}`
  );
  return await fetch(req);
}

export async function fetchAllUserCollection(
  username: string,
  pageSize: number = 50
...
export async function getSubjectInfo(
  kv: KVNamespace,
  subjectID: number
): Promise<SlimSubject | null> {
  const key = `subject-v8-${subjectID}`;
  let value = await kv.get(key, KVReadOption);
  if (value) {
...
export async function fetchAllEpisode(
  subjectID: number
  ...
  return res
    .map((episode) => {
      ...
      if (date[0] === null || date[1] === null || date[2] === null) {
        throw new Error(
          `failed to parse episode for ${subjectID} ${episode.id}, ${episode.airdate}`
        );
      }
